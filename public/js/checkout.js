...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function prefillUserInfo() {
    try {
      ...
      sdtInput.value = user.so_dien_thoai || "";
  ...
  checkoutForm.addEventListener("submit", async (event) => {
    ...
    const selectedIdsJSON = sessionStorage.getItem("selectedCartItemIds");
    const selectedCartItemIds = selectedIdsJSON
      ? JSON.parse(selectedIdsJSON)
      : [];
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Server returned non-JSON response: ${textData.substring(0, 100)}...`,
        );
      }
