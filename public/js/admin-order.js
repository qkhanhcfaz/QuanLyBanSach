...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // === TỰ ĐỘNG CẬP NHẬT KHI QUAY LẠI TRANG (Back Button) ===
  window.addEventListener("pageshow", (event) => {
    // event.persisted = true nếu trang được load từ BFCache (Back-Forward Cache)
    // Hoặc kiểm tra navigation.type nếu cần (nhưng nav.type bị deprecated)
    // Đôi khi cần reload dù không phải persisted để đảm bảo dữ liệu mới nhất
    if (
      event.persisted ||
      ...
      const page = parseInt(
        document.querySelector(".page-item.active .page-link")?.dataset.page ||
          1,
      );
      const keyword = keywordInput.value.trim();
      const status = statusSelect.value;
