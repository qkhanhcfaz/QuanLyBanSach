document.addEventListener("DOMContentLoaded", function () {
    ...
    const renderTree = (cats, level) => {
      ...
      cats.forEach((cat) => {
        tableBody.insertAdjacentHTML(
          "beforeend",
          createCategoryRowHTML(cat, level),
        );
  ...
  function createCategoryRowHTML(category, level) {
    ...
    const productCount =
      ...
      (category.dataValues ? category.dataValues.productCount : 0);

    const isFeatured = category.is_featured ? "checked" : "";
            ...
            <tr data-id="${category.id}">
                ...
                <td class="text-center">
                    <div class="form-check form-switch d-flex justify-content-center">
                        <input class="form-check-input btn-toggle-featured" type="checkbox" ${isFeatured}>
                    </div>
                </td>
  ...
  function renderCategoryTableFlat(categories) {
    ...
    if (!categories || categories.length === 0) {
      tableBody.innerHTML =
        '<tr><td colspan="5" class="text-center">Không tìm thấy danh mục nào phù hợp.</td></tr>';
      return;
    }
  ...
  async function toggleFeaturedStatus(id, isFeatured) {
    try {
      ...
      const result = await response.json();
      // Optional: Show small toast or just silent update
      ...
      // Revert checkbox state if failed (would need reference to checkbox)
  ...
  document
    ...
    .addEventListener("click", prepareCreateForm);
  ...
  tableBody.addEventListener("change", (e) => {
    if (e.target.classList.contains("btn-toggle-featured")) {
      const id = e.target.closest("tr").dataset.id;
      const isFeatured = e.target.checked;
      toggleFeaturedStatus(id, isFeatured);
    }
  });
