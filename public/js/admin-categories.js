document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderCategoryTable(categories) {
    ...
    parentCategories.sort((a, b) =>
      a.ten_danh_muc.localeCompare(b.ten_danh_muc),
    );
    ...
    parentCategories.forEach((parent) => {
      ...
      tableBody.insertAdjacentHTML(
        "beforeend",
        createCategoryRowHTML(parent, 0),
      );

      // Bước 3: Tìm TẤT CẢ các danh mục con của nó
      const childCategories = categories.filter(
        (child) => child.danh_muc_cha_id === parent.id,
      );
  ...
  function createCategoryRowHTML(category, level) {
    ...
    const parentName = category.parentCategory
      ? category.parentCategory.ten_danh_muc
      : "(Không có)";
    const productCount =
