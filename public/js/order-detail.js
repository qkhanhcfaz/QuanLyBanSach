...
function renderOrderDetail(data) {
  ...
  if (
                ...
                <tr>
                    <td>
                        <div class="d-flex align-items-center">
                            <img src="${item.product.img && (item.product.img.startsWith("/") || item.product.img.startsWith("http")) ? item.product.img : "/images/" + (item.product.img || "default-book.png")}"
                                 alt="${productName}"
                                 class="img-thumbnail me-3"
                                 style="width: 100px; height: 140px; object-fit: cover;"
                                 onerror="this.onerror=null; this.src='/images/default-book.png';">
                            <div>
                                <h6 class="mb-0 text-primary fw-bold" style="font-size: 0.95rem;">${productName}</h6>
                            </div>
                        </div>
                    </td>
                    <td class="text-center align-middle">${item.so_luong_dat}</td>
                    <td class="text-end align-middle">${productPrice}</td>
                    <td class="text-end align-middle fw-bold">${lineTotal}</td>
                </tr>
            `;
    ...
    itemsContainer.innerHTML =
      '<tr><td colspan="4" class="text-center">Không có sản phẩm nào trong đơn hàng này.</td></tr>';
  }
