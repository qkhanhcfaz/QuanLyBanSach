...
async function loadCategoriesForMenu() {
  ...
  try {
    ...
    menuList.innerHTML =
      '<li><a class="dropdown-item text-danger" href="#">Lỗi tải danh mục</a></li>';
  }
}
...
function loadHomePageData() {
  ...
  // fetchNewArrivals(); // Đã chuyển sang SSR
  fetchPostsAndRender();
...
async function fetchProductsAndRender(wrapperId, pathAndQuery) {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        `API call failed for ${apiUrl} with status ${response.status}`,
      );
    ...
    if (products && products.length > 0) {
                            ...
                            <div class="d-flex gap-2">
                                ...
                                ${(() => {
                                  const isFav =
                                    ...
                                    window.favoriteProductIds.includes(
                                      product.id,
                                    );
      ...
      wrapper.innerHTML =
        '<div class="text-muted p-3">Không có sản phẩm nào thuộc mục này.</div>';
    }
  } catch (error) {
