...
const createPost = async (req, res) => {
  try {
    ...
    let hinh_anh = req.body.hinh_anh; // URL text input (fallback)

    if (req.file) {
      hinh_anh = `/uploads/posts/${req.file.filename}`;
    }
    ...
    res
      .status(201)
      .json({ message: "Tạo bài viết thành công!", post: newPost });
  } catch (error) {
...
const updatePost = async (req, res) => {
  try {
    ...
    // Only update hinh_anh if a new one is uploaded or provided (if URL input supported)
