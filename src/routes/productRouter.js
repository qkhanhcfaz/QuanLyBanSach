...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    // Đặt tên file: fieldname-timestamp.extension
...
const upload = multer({
  ...
  fileFilter: function (req, file, callback) {
    ...
    if (
      ...
      return callback(
        new Error("Chỉ cho phép upload ảnh (png, jpg, gif, jpeg, webp)"),
      );
    }
    callback(null, true);
  },
...
const createProduct = async (request, response) => {
  try {
    ...
    if (!ten_sach || !gia_bia || !danh_muc_id) {
      return response
        .status(400)
        .json({ message: "Tên sách, Giá bìa, và Danh mục là bắt buộc." });
    }
...
const getAllProducts = async (request, response) => {
  try {
    ...
    const { count, rows } = await Product.findAndCountAll({
      ...
      include: {
        // Join với bảng Category để lấy tên danh mục
        model: Category,
        as: "category",
...
const updateProduct = async (request, response) => {
  try {
    ...
    if (error.name === "SequelizeValidationError") {
      const messages = error.errors.map(function (e) {
        return e.message;
      });
      return response.status(400).json({
...
const getAllPublishers = async (req, res) => {
  try {
    const publishers = await Product.findAll({
      attributes: [
        [
          sequelize.fn("DISTINCT", sequelize.col("nha_xuat_ban")),
          "publisher_name",
        ],
      ],
    ...
    res
      .status(500)
      .json({ message: "Lỗi server khi lấy danh sách nhà xuất bản" });
  }
};
// === ĐỊNH NGHĨA CÁC ROUTES ===

// GET /api/products/bestsellers - Lấy sản phẩm bán chạy nhất từ database
...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Lỗi khi lấy sản phẩm", chi_tiet: error.message });
  }
});

// POST /api/products - Tạo sản phẩm mới (Có upload ảnh, chỉ Admin)
