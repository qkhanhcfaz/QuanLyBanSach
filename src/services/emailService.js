...
const sendOrderConfirmationEmail = async (toEmail, orderDetails) => {
  ...
  try {
    ...
    console.log(
      `✅ Email xác nhận đơn hàng #${orderDetails.id} đã được gửi thành công tới ${toEmail}`,
    );
  } catch (error) {
    ...
    console.error(
      `❌ Lỗi khi gửi email xác nhận đơn hàng #${orderDetails.id} tới ${toEmail}: `,
      error,
    );
  }
};
...
const sendOutOfStockApologyEmail = async (toEmail, customerInfo, product) => {
  ...
  try {
    await transporter.sendMail(mailOptions);
    console.log(
      `✅ Email xin lỗi hết hàng cho sản phẩm #${product.id} đã được gửi tới ${toEmail}.`,
    );
  } catch (error) {
    console.error(
      `❌ Lỗi khi gửi email xin lỗi hết hàng tới ${toEmail}:`,
      error,
    );
  }
};
